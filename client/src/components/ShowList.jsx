...
const ShowList = () => {
  ...
  const fetchShows = useCallback(async () => {
    try {
      ...
      // Get sample houses
      const sampleHouses = getAllHouses();
      allShows = [...sampleHouses];
      ...
      try {
        ...
        const apiShows = response.data.items || response.data;
        // Merge API data with sample data (avoid duplicates by checking ID)
        const existingIds = allShows.map(show => show.SHOW_ID);
        const newApiShows = apiShows.filter(show => !existingIds.includes(show.SHOW_ID));
        allShows = [...allShows, ...newApiShows];
      } catch (apiError) {
              ...
              <Card className="show-card">
                ...
                <Card.Body>
                  ...
                  <Card.Text>
                    ...
                    {show.owner && (
                      <span>
                        <strong>Owner:</strong> {show.owner.name}
                        {show.owner.verified && (
                          <Badge bg="success" className="ms-1" style={{ fontSize: '0.7em' }}>Verified</Badge>
                        )}
                        <br />
                      </span>
                    )}
                ...
                <Card.Footer className="d-flex justify-content-between">
                  <div>
                    ...
                    {/* Contact Owner Button for Users */}
                    {user?.role === 'user' && (
                      <Button 
                        variant="primary" 
                        className="btn-action me-2"
                        as={Link}
                        to={`/property/${show.SHOW_ID}`}
                      >
                        <i className="fas fa-user me-1"></i>Contact Owner
                      </Button>
                    )}
                  </div>
                  <div>
